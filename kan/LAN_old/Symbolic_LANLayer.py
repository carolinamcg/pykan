...
class Symbolic_LANLayer(nn.Module):
    '''
    LANLayer class
    ...
    --------
        __init__(): 
            initialize a Symbolic_LANLayer
        forward():
            forward
        get_subset():
            get subset of the LANLayer (used for pruning)
        fix_symbolic():
            fix an activation function to be symbolic
    '''
    def __init__(self, in_dim=3, device='cpu'):
        '''
        initialize a Symbolic_LANLayer (activation functions are initialized to be identity functions)
        ...
        Example
        -------
        >>> sb = Symbolic_LANLayer(in_dim=3, out_dim=3)
        >>> len(sb.funs), len(sb.funs[0])
        (3, 3)
        '''
        super(Symbolic_LANLayer, self).__init__()
        #self.out_dim = out_dim
        self.in_dim = in_dim
        self.mask = torch.nn.Parameter(torch.zeros(in_dim, device=device)).requires_grad_(False)
        # torch
        self.funs = [lambda x: x for i in range(self.in_dim)]
        # name
        self.funs_name = ['' for i in range(self.in_dim)]
        # sympy
        self.funs_sympy = ['' for i in range(self.in_dim)]
        
        self.affine = torch.nn.Parameter(torch.zeros(in_dim, 4, device=device))
        # c*f(a*x+b)+d
    ...
    def forward(self, x):
        ...
        Example
        -------
        >>> sb = Symbolic_LANLayer(in_dim=3, out_dim=5)
        ...
        for i in range(self.in_dim):
            ...
            xj = self.affine[i,2]*self.funs[i](self.affine[i,0]*x[:,[i]]+self.affine[i,1])+self.affine[i,3]
            postacts_.append(self.mask[i]*xj)
            postacts.append(postacts_)

        postacts = torch.stack(postacts)
        postacts = postacts.permute(1,0,2)[:,:,0]#.permute(2,1,0,3)[:,:,:,0] #(batch, in_dim, out_dim)
        y = postacts.clone() #torch.sum(postacts, dim=2) #(batch, out_dim)
    ...
    def get_subset(self, in_id, out_id):
        '''
        get a smaller Symbolic_LANLayer from a larger Symbolic_LANLayer (used for pruning)
        ...
        Returns:
        --------
            spb : Symbolic_LANLayer
         
        Example
        -------
        >>> sb_large = Symbolic_LANLayer(in_dim=10, out_dim=10)
        >>> sb_small = sb_large.get_subset([0,9],[1,2,3])
        >>> sb_small.in_dim, sb_small.out_dim
        (2, 3)
        '''
        sbb = Symbolic_LANLayer(self.in_dim, self.out_dim, device=self.device)
    ...
    def fix_symbolic(self, i, j, fun_name, x=None, y=None, random=False, a_range=(-10,10), b_range=(-10,10), verbose=True):
        ...
        >>> sb = Symbolic_LANLayer(in_dim=3, out_dim=2)
