...
class LossLearnerLAN(nn.Module):
    """
    LAN class
    ...
    def __init__(
        ...
        self.biases = []
        self.act_fun = []
        self.depth = 2 #1st layer = activation function; 2nd = loss
        self.width = [n_neurons]*3
        ...
        for l in range(self.depth):
            ...
            sp_batch = KANLayer(
                ...
                device=device,
                allweights_sharing=False if l==0 else True, # learned loss function should be the same for all neurons 
                LAN=True,
            )
            self.act_fun.append(sp_batch)

            # bias
            bias = nn.Linear(self.width[l + 1], 1, bias=False, device=device).requires_grad_(
                bias_trainable
            )
            bias.weight.data *= 0.0 #biases are initialized w/ 0
        ...
        for l in range(self.depth):
            sb_batch = Symbolic_KANLayer(
                in_dim=self.width[l], out_dim=self.width[l + 1], device=device, LAN=True
            )
            self.symbolic_fun.append(sb_batch)

        self.symbolic_fun = nn.ModuleList(self.symbolic_fun)
        self.symbolic_enabled = (
            symbolic_enabled  # self.symbolic_func is always initialized, but
        )
        # then it may not be used in the forward. Why? Maybe, for purposes of easier transfer learning
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        Example
        -------
        >>> model_coarse = KAN(self.width=[2,5,1], grid=5, k=3)
        >>> model_fine = KAN(self.width=[2,5,1], grid=10, k=3)
        ...
        self.initialize_grid_from_another_model(
    ...
    def forward(self, x):
        ...
        KAN forward
        ---> For each layer:
            1. Act_fun computes/learns the b() + spline() activation per in_neuron*out_neuron
            2. If symbolic enabled, symbolic_fun computes/learns 4 coefficients per in_neuron*out_neuron: c*f(a*x + b) + d
            3. The outputs of both layers are summed (batch, out_neurons)
            4. A learnable biases is added to the final outputs
        ...
        Example
        -------
        >>> model = KAN(self.width=[2,5,3], grid=5, k=3)
        >>> x = torch.normal(0,1,size=(100,2))
        >>> model(x).shape
        torch.Size([100, 3])
        """
        ...
        self.acts.append(x)  # acts shape: (batch, self.width[l])=(batch, in_neurons)

        for l in range(self.depth):

            x_numerical, preacts, postacts_numerical, postspline = self.act_fun[l](x)
            # act_fun learns each spline activation function per in_neuron*out_neuron
            # as a sum of G+k spline basis functions.
            # x_numerical=final output of the KAN activation layer (batch, out_neurons)
            # where out_neurons are the summed value of each input dim spline

            if self.symbolic_enabled == True:
                x_symbolic, postacts_symbolic = self.symbolic_fun[l](x)
                # learns, for each in_neuron*out_neuron, 4 coefficients: c*f(a*x + b) + d
                # from input x (batch, in_neurons)
            else:
                x_symbolic = 0.0
            ...
            postacts = (
                postacts_numerical + postacts_symbolic
            )  # (batch, out_neurons, in_neurons)
            ...
            if self.act_fun[l].allweights_sharing: #allweights shared, one spline is learned for all neurons in LAN layer
                ...
                grid_reshape = self.act_fun[l].grid.reshape(1, self.self.width[l], -1)
            ...
            x = x + self.biases[l].weight  # (batch, out_neurons)
            self.acts.append(x)
    ...
    def fix_symbolic(
        ...
        >>> model = KAN(self.width=[2,5,1], grid=5, k=3, noise_scale=1.)
        >>> x = torch.normal(0,1,size=(100,2))
    ...
    def unfix_symbolic_all(self):
        ...
        """
        for l in range(len(self.self.width) - 1):
            for i in range(self.self.width[l]):
                for j in range(self.self.width[l + 1]):
    ...
    def lock(self, l, ids):
        ...
        Example
        -------
        >>> model = KAN(self.width=[2,3,1], grid=5, k=3, noise_scale=1.)
    ...
    def get_range(self, l, i, j, verbose=True):
        ...
        """
        x = self.spline_preacts[l][
            :, j, i
        ]  # the input x to teh layer reshaped to (batch, self.out_dim, self.in_dim)
        y = self.spline_postacts[l][:, j, i]
        x_min = torch.min(x)
        x_max = torch.max(x)
        y_min = torch.min(y)
    ...
    def plot_LAN(
        ...
        -----
            ...
            title: None or str
                title
            plot_full : bool
                if True, plots the full model, with all spline/activation images together. Otherwise, just plots and saves the individual splines/activations
        ...
        >>> model = KAN(self.width=[2,3,1], grid=3, k=3, noise_scale=1.0)
        >>> x = torch.normal(0,1,size=(100,2))
        >>> model(x) # do a forward pass to obtain model.acts
        >>> model.plot()
        """
        ...
        for l in range(depth):
            ...
            for i in range(self.self.width[l]):
                ...
                numerical_mask = self.act_fun[l].mask.reshape(1, self.self.width[l])[j][i]
                ...
                if tick == True:
                    ax.tick_params(
                        axis="y", direction="out", labelrotation=90, pad=3, labelsize=4
                    )
                    ax.tick_params(axis="x", direction="out", pad=1, labelsize=4)
                    x_min, x_max, y_min, y_max = self.get_range(l, i, j, verbose=False)
                    plt.xticks([x_min, x_max], ["%4.4f" % x_min, "%4.4f" % x_max])
                    plt.yticks([y_min, y_max], ["%4.4f" % y_min, "%4.4f" % y_max])
                else:
                    plt.xticks([])
                    plt.yticks([])
                if alpha_mask == 1:
                ...
                plt.gca().patch.set_lineself.width(1.5)
                # plt.axis('off')
                ...
                lock_id = self.act_fun[l].lock_id[j * self.self.width[l] + i].long().item()
        ...
        if plot_full:
            ...
            # draw skeleton
            self.width = np.array(self.self.width)
            A = 1
            y0 = 0.4  # 0.4

            # plt.figure(figsize=(5,5*(neuron_depth-1)*y0))
            neuron_depth = len(self.width)
            min_spacing = A / np.maximum(np.max(self.width), 5)

            max_num_weights = np.max(self.width)
            ...
            for l in range(neuron_depth):
                ...
                for i in range(n):
                    ...
                    if l < neuron_depth - 1:  # if it's not the last layer
                        # plot connections
                        n_next = self.width[l + 1]
                        N = n
                        # for j in range(n_next):
                        j = 0
                        id_ = i

                        symbol_mask = self.symbolic_fun[l].mask[0][i]
                        numerical_mask = self.act_fun[l].mask.reshape(1, self.self.width[l])[
                            j
                        ][i]
                        ...
                        if mask == True:
                            ...
                            # plot connections from in_neurons (pre_activations) to spline functions
                            plt.plot(
                                [1 / (2 * n) + i / n, 1 / (2 * N) + id_ / N],
                                [l * y0, (l + 1 / 2) * y0 - y1],
                                color=color,
                            ...
                            # plot connections from spline functions to out_neurons (post_activations)
                            plt.plot(
                                [
                                    1 / (2 * N) + id_ / N,
                                    1 / (2 * n_next) + id_ / n_next,
            ...
            for l in range(neuron_depth - 1):
                ...
                for i in range(n):
                    n_next = self.width[l + 1]
                    N = n  # NUMBER OF SPLINES/CONNECTIONS
                    # for j in range(n_next):
                    j = 0
                    id_ = i
                    ...
                    if mask == False:
                        newax.imshow(im, alpha=1)  # alpha[l][j][i])
                    else:
    ...
    def train(
        ...
        >>> model = KAN(self.width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0)
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        active_neurons = [list(range(self.self.width[0]))]
        for i in range(len(self.acts_scale) - 1):
            if mode == "auto":
                ...
                overall_important = torch.zeros(self.self.width[i + 1], dtype=torch.bool)
        ...
        active_neurons.append(list(range(self.self.width[-1])))
        mask.append(
            torch.ones(
                self.self.width[-1],
            )
        ...
        for l in range(len(self.acts_scale) - 1):
            for i in range(self.self.width[l + 1]):
        ...
        for i in range(len(self.acts_scale)):
            ...
            model2.self.width[i] = len(active_neurons[i])
    ...
    def remove_edge(self, l, i, j):
        ...
        --------
            None
        """
        self.act_fun[l].mask[j * self.self.width[l] + i] = 0.0
    ...
    def remove_node(self, l, i):
        ...
        self.act_fun[l - 1].mask[
            i * self.self.width[l - 1] + torch.arange(self.self.width[l - 1])
        ] = 0.0
        self.act_fun[l].mask[torch.arange(self.self.width[l + 1]) * self.self.width[l] + i] = 0.0
    ...
    def auto_symbolic(self, a_range=(-10, 10), b_range=(-10, 10), lib=None, verbose=1):
        ...
        """
        for l in range(len(self.self.width) - 1):  # layers
            for i in range(self.self.width[l]):  # input neurons
                j=0 #CHANGED
    ...
    def symbolic_formula(
        ...
        Example
        -------
        >>> model = KAN(self.width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0, grid_eps=0.02)
        >>> f = lambda x: torch.exp(torch.sin(torch.pi*x[:,[0]]) + x[:,[1]]**2)
        >>> dataset = create_dataset(f, n_var=2)
        >>> model.train(dataset, opt='LBFGS', steps=50, lamb=0.01);
        >>> model = model.prune()
        ...
        # define variables
        if var == None:
            for ii in range(1, self.self.width[0] + 1):
        ...
        for l in range(len(self.self.width) - 1):
            y = []
            for i in range(self.self.width[l]):  # no sum, relation is 1-to-1 in LAN
                yj = 0.0
                j = 0
                ...
                try:
                    ...
                    return
                if simplify == True:
                    y.append(sympy.simplify(yj + self.biases[l].weight.data[0, i]))
                else:
                    y.append(yj + self.biases[l].weight.data[0, i])
        ...
        # x0 is a list of the names of each of the in_neurons/variables
        # first output is a list of the symbolic expression in the last layer
        return [
